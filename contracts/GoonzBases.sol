...
contract GoonzBases is Initializable, ERC721Upgradeable, ERC1155HolderUpgradeable,  OwnableUpgradeable {
    ...
    uint256 public giveawaysClaimed;
...
/// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function isGiveawayWinner(address _user) public view returns(bool winner){
        if(giveawayTracker[_user].amount > 0 && giveawayTracker[_user].numberClaimed < giveawayTracker[_user].amount){
            return true;
        } else {
            return false;
        }
     ...
     function giveawayWinnerAmount(address _user) public view returns(uint256 amount){
        return giveawayTracker[_user].amount;
    }

     function giveawayWinnerClaimedAmount(address _user) public view returns(uint256 amount){
        return giveawayTracker[_user].numberClaimed;
    }
    ...
    function getTotalSupply() public view returns(uint256 total_supply){
     ...
     function getTotalGiveaway() public view returns(uint256 total_supply){
